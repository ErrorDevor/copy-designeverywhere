...
import type { SerializedInlineBlockNode } from "../types/nodeTypes";
...
export function convertLexicalNodesToJSX({
  ...
  const unknownConverter: JSXConverter<any> =
    converters.unknown as JSXConverter<any>;

  const jsxArray: React.ReactNode[] = nodes.map((node, i) => {
    let converterForNode: JSXConverter<any> | undefined;
    if (node.type === "block") {
      converterForNode = converters?.blocks?.[(node as any)?.fields?.blockType];
      if (!converterForNode && !unknownConverter) {
        console.error(
          `Lexical => JSX converter: Blocks converter: found ${(node as any)?.fields?.blockType} block, but no converter is provided`
        );
      }
